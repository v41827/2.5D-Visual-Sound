...
def main():
    ...
    ap.add_argument("--output_root", default="/scratch/yc01847/AVT_demo_output",help="Where to write demo outputs")

    ap.add_argument("--num_splits", type=int, default=10)
    ap.add_argument("--text_folder_name", default="sampled_output_step10")

    # Split-specific checkpoints are derived automatically under this root
    ap.add_argument("--ckpt_root", default="/scratch/yc01847/2.5D-Visual-Sound/checkpoints/AVT_text_frozen",help="Root containing split{k}/mono2binaural/{visual,audio}_best.pth")

    # Demo/runtime knobs
    ap.add_argument("--freeze_text", default="True")
    ...
    text_dir     = os.path.join(args.fairplay_root, args.text_folder_name)
    ...
    for k in range(1, args.num_splits + 1):
        ...
        for id_ in ids:
            ...
            test_text_path = os.path.join(text_dir, f"{id_}.csv")

            # one output dir per (split, id)
            output_dir_root = os.path.join(args.output_root, f"AVTSplit{k}_{id_}")
            ...
            cmd = [
                ...
                "--text_folder_name", args.text_folder_name,
